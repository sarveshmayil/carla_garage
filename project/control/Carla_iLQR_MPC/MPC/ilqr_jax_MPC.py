...
def continuous_dynamics(state, action):
    ...
    phi_yf=(1-Ey)*(a_f+Shy)+(Ey/By)*np.arctan2(By*(a_f+Shy))
    phi_yr=(1-Ey)*(a_r+Shy)+(Ey/By)*np.arctan2(By*(a_r+Shy))

    F_zf=b/(a+b)*m*g
    F_yf=F_zf*Dy*np.sin(Cy*np.arctan2(By*phi_yf))+Svy

    F_zr=a/(a+b)*m*g
    F_yr=F_zr*Dy*np.sin(Cy*np.arctan2(By*phi_yr))+Svy
    ...
    dzdt = [state[1]*np.cos(state[4]) - state[3]*np.sin(state[4]), 
            ...
            state[2]*np.sin(state[4]) + state[3]*np.cos(state[4]), 
